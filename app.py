...
def estimate_bpm(y: np.ndarray, sr: int) -> float:
    try:
        ...
        return 120.0  # Default BPM se non riesce a stimare

class VideoGenerator:
    def __init__(self, format_res: Tuple[int, int], level: str, fps: int = 30,
                 line_color=(0,0,0), bg_color=(255,255,255), effect_type="lines", bpm:float=120.0):
    ...
    def energy_to_color(self, energy: float) -> Tuple[int, int, int]:
        # Linee sempre nere, ma puoi personalizzare qui con energy se vuoi
    ...
    def draw_connected_network(self, frame, num_nodes, time_index, mel_spec_norm):
        ...
        for i in range(len(points)):
            for j in range(i+1, len(points)):
                ...
                if energy > 0.3:
                    cv2.line(frame, points[i], points[j], self.energy_to_color(energy), 1)

    def draw_spectrum(self, frame, mel_spec_norm, time_index):
        height = frame.shape[0]
        width = frame.shape[1]
        bands = mel_spec_norm.shape[0]
        # Per soft, solo linee verticali; medium orizzontali; hard entrambe
        if self.LEVEL == "soft":
    ...
    def draw_barcode(self, frame, mel_spec_norm, time_index):
        ...
        width = frame.shape[1]
        bands = mel_spec_norm.shape[0]
        bpm_factor = self.bpm / 120.0  # riferimento 120 bpm
        bar_width = max(1, width // (bands*2))
        max_bar_height = height

        for i in range(bands):
            energy = mel_spec_norm[i, time_index]
            if energy > 0.05:
                # Spessore linee per frequenze basse più larghe
                if i < bands // 3:
                    thickness = int(5 * energy * bpm_factor)
                elif i < 2 * bands // 3:
                    thickness = int(3 * energy * bpm_factor)
                else:
                    thickness = int(1 * energy * bpm_factor)
                thickness = max(1, thickness)

                # Altezza barre legata energia (volumi)
                bar_height = int(max_bar_height * energy)

                # Posizione verticale (base in basso)
                x = i * bar_width * 2
                y_start = height - bar_height

                color = self.energy_to_color(energy)
                # Disegna la linea verticale stile barcode
                cv2.line(frame, (x, y_start), (x, height), color, thickness)
...
def main():
    ...
    if uploaded_file is not None:
        ...
        fps = st.selectbox("⏱️ Seleziona FPS", fps_options, index=2)

        col1, col2 = st.columns(2)
        with col1:
            video_format = st.selectbox("📐 Formato video", list(FORMAT_RESOLUTIONS.keys()), index=0)
        with col2:
            effect_type = st.selectbox("🎨 Tipo effetto", ["lines", "spectrum", "barcode"], index=0)

        effect_level = st.selectbox("⚙️ Livello effetti", ["soft", "medium", "hard"], index=0)
        ...
        bgr_line_color = hex_to_bgr(line_color)
        bgr_bg_color = hex_to_bgr(bg_color)
        ...
        if st.button("🎬 Genera Video"):
            generator = VideoGenerator(
                ...
                effect_level,
                fps,
                line_color=bgr_line_color,
                bg_color=bgr_bg_color,
